...
static void mat3w_put(
    FILE *s,
    cp_mat3w_t const* a)
{
    for (cp_size_each(y, 3)) {
        for (cp_size_each(x, 3)) {
            fprintf(s, " %7.4f", a->b.m[y][x]);
        }
        fprintf(s, " %7.4f\n", a->w.v[y]);
    }
}
...
extern void cp_math_test(void)
{
    ...
    {
        /* Very small facet, and expected normal is -1,0,0
         * Needs _x variant to work.
         */
        cp_vec3_t n = {{ -1, 0, 0 }};
        cp_vec3_t a = {{ -87.756103515625, 18.34033203125,   40.50000000000001 }};
        cp_vec3_t b = {{ -87.756103515625, 18.3402099609375, 40.69 }};
        cp_vec3_t c = {{ -87.756103515625, 18.34033203125,   40.69 }};

        cp_vec3_t p;
        bool ok = cp_vec3_left_normal3_x(&p, &a, &b, &c);
        TEST_EQ(ok, true);
        TEST_FEQ(n.x, p.x);
        TEST_FEQ(n.y, p.y);
        TEST_FEQ(n.z, p.z);
